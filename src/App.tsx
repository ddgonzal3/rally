        ...
        {activePrs.length > 0 && (
          <div style={styles.titlebarRight}>
            {activePrs.map(({ path, repoName, pr }) => (
              ...
              {/* GitHub icon — opens repo URL in browser */}
              <button
                className="activity-btn"
                style={{ ...styles.prPill, padding: "2px 4px" }}
                onClick={(e) => {
                  e.stopPropagation();
                  // Derive repo URL from PR URL: strip /pull/123
                  const repoUrl = pr.url.replace(/\/pull\/\d+$/, "");
                  openUrl(repoUrl);
                }}
                title={`Open ${repoName} on GitHub`}
              >
                <svg width="14" height="14" viewBox="0 0 24 24" fill="#ddd" style={{ flexShrink: 0 }}>
