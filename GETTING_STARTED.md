...
Rally is a native macOS app for orchestrating multiple Claude Code sessions, git workflows, and dev processes across repo workspaces. Think of it as a mission control for your development workflow.

---

## 1. Creating Workspaces

A workspace is Rally's core unit — it groups one or more repo folders together with their own terminals, git state, and pane layout.
...
## 2. Multiple Checkouts (Worktrees)
...
## 3. Panes and Terminals

Rally uses a flexible pane system. Each workspace has its own layout of terminals, editors, diffs, and more.

### Keyboard Shortcuts

| Shortcut | Action |
|----------|--------|
| `Cmd+/` | Split active pane right (adds a new terminal) |
| `Ctrl+`` ` | Toggle bottom panel (creates/collapses a vertical split) |
| `Cmd+W` | Close the active pane/tab |
...
| `Option+Left/Right` | Word movement in terminals |

### Pane Types

- **Terminal** — Real PTY-backed shell, same as your regular terminal
- **Claude Pane** — Claude Code session running inside Rally
...
The file explorer appears in the left panel next to the sidebar.
...
2. Add related repos via **File > Add Folder to Workspace**
...
1. Open a Claude pane (click a built-in command, or split a pane and launch Claude)
...
- **Use `Shift+Arrow`** to navigate between panes instead of clicking — it's much faster
- **`Ctrl+`` `** is great for a quick bottom terminal while you have editors open above
