...
Builds are signed with the machine's own Apple Development certificate. The requirement becomes `identifier "com.rally.app"` plus the certificate name, which survives rebuilds (and renewal, as long as the certificate keeps the same name). After changing the identity once, reset the stale rows (`tccutil reset Accessibility com.rally.app`, same for `ScreenCapture`) and re-add Rally.

**Never put `signingIdentity` in `tauri.conf.json`.** A certificate lives in one person's keychain; committing Danny's broke every other teammate's build with "no identity found". Every build goes through `scripts/tauri-build.sh`, which uses `$APPLE_SIGNING_IDENTITY` if set, else the first "Apple Development" identity in the keychain, else builds unsigned with a warning (works, but grants reset on each rebuild). Never call `cargo tauri build` directly.
